...
export function parseANZ(csv: string): ParsedRow[] {
...
export function detectANZ(csv: string): boolean {
  const h = csv.slice(0, 200).toLowerCase()
  return h.includes('anz') || (h.includes('date') && h.includes('details') && !h.includes('narrative'))
}
